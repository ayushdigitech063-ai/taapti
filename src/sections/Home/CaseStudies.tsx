...
export default function CaseStudies() {
  const [data, setData] = useState({
    ...
    description:
      "From ambitious startups to global enterprises, we build scalable software solutions that solve real-world problems.",
      ...
      if (res && res.ok) {
        ...
        if (json && json.success && json.data) {
          ...
          setData({
            ...
            description: d.description || "From ambitious startups to global enterprises, we build scalable software solutions that solve real-world problems.",
            caseStudies: d.caseStudies && d.caseStudies.length > 0 ? d.caseStudies : defaultCaseStudies,
          });
        }
      }
    } catch {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && "BroadcastChannel" in window) {
      ...
      channel.onmessage = (event) => {
        if (event.data === "CASE_STUDIES_UPDATED" || event.data === "CMS_UPDATED") {
          fetchCaseStudies();
        }
      };
    }
      ...
      <div className="container tp-case-studies__container">
        ...
        <div className="tp-case-studies__header reveal-down">
          <div className="tp-case-studies__header-content text-center">
            <div className="tp-case-studies__eyebrow" style={{ color: "#00875A" }}>
              <span className="tp-eyebrow-line" style={{ background: "#00875A" }}></span>
              <span>{data.eyebrow}</span>
              <span className="tp-eyebrow-line" style={{ background: "#00875A" }}></span>
            </div>

            <h2 className="tp-case-studies__title">
              {data.headingNormal} <span className="tp-text-highlight" style={{ color: "#10243E" }}>{data.headingHighlight}</span>
            </h2>
        ...
        <div className="tp-cards-grid reveal-zoom">
          {data.caseStudies.map((cs, idx) => {
            ...
            const isDarkCard = cs.isDark || idx === 1;

            if (idx === 0) {
              return (
                /* CARD 1: FINTECH */
                <article key={cs.id || idx} className="tp-card tp-card--light tp-card--fintech">
                  <div className="tp-card__top">
                    <span className="tp-badge tp-badge--fintech">{cs.category}</span>
                    <span className="tp-card__number">{num}</span>
                  </div>

                  <div className="tp-card__body">
                    <h3 className="tp-card__title">{cs.title}</h3>
                    <p className="tp-card__desc">{cs.description}</p>

                    <Link href={cs.link || "/case-studies"} className="tp-card__action">
                        ...
                        <div className="tp-phone">
                          ...
                          <div className="tp-phone__screen">
                            ...
                            <div className="tp-phone__actions">
                              <div className="tp-phone__act-btn"><span className="tp-act-icon">↑</span><span>Send</span></div>
                              <div className="tp-phone__act-btn"><span className="tp-act-icon">↓</span><span>Receive</span></div>
                              <div className="tp-phone__act-btn"><span className="tp-act-icon">✕</span><span>Invest</span></div>
                              <div className="tp-phone__act-btn"><span className="tp-act-icon">••</span><span>More</span></div>
                            </div>
                            ...
                            <div className="tp-phone__tx-section">
                              ...
                              <div className="tp-tx-item">
                                ...
                                <div className="tp-tx-info"><strong>Zomato</strong><small>Aug 12, 2024</small></div>
                                <span className="tp-tx-amount">-₹1,240</span>
                              </div>
                              <div className="tp-tx-item">
                                <div className="tp-tx-logo tp-tx-logo--spotify">🟢</div>
                                <div className="tp-tx-info"><strong>Spotify</strong><small>Aug 11, 2024</small></div>
                  ...
                  <div className="tp-card__footer">
                    {cs.metrics?.map((m, mIdx) => (
                      <div key={mIdx} className="tp-metric">
                        <span className="tp-metric__val">{m.value}</span>
                        <span className="tp-metric__lbl">{m.label}</span>
                      </div>
                    ))}
                  </div>
                </article>
              ...
              return (
                /* CARD 2: HEALTHCARE (DARK CARD) */
                <article key={cs.id || idx} className="tp-card tp-card--dark tp-card--healthcare">
                  <div className="tp-card__top">
                    <span className="tp-badge tp-badge--healthcare">{cs.category}</span>
                    <span className="tp-card__number">{num}</span>
                  </div>
                      ...
                      <div className="tp-watch-wrapper">
                        <div className="tp-watch">
                          <div className="tp-watch__screen">
                            ...
                            <div className="tp-watch__bpm"><strong>72</strong><span>bpm</span></div>
                            <div className="tp-watch__ecg">
                              <svg viewBox="0 0 120 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M0 15 H35 L40 5 L45 25 L50 10 L55 20 L60 15 H120" stroke="#14b8a6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                              </svg>
                            </div>
                          </div>
                        </div>
              ...
              return (
                /* CARD 3+: SAAS LIGHT CARD */
                <article key={cs.id || idx} className="tp-card tp-card--light tp-card--saas">
                  <div className="tp-card__top">
                    <span className="tp-badge tp-badge--saas">{cs.category}</span>
                    <span className="tp-card__number">{num}</span>
                  </div>
